...
class channels:
    ...
    def items_list(self, i):
        try:
            ...
            director = writer = ''
            if 'beanies' in people and 'directing' in people['beanies']:
                director = ', '.join([director['person']['name'] for director in people['beanies']['directing'] if director['job'].lower() == 'director'])
            if 'beanies' in people and 'writing' in people['beanies']:
                writer = ', '.join([writer['person']['name'] for writer in people['beanies']['writing'] if writer['job'].lower() in ['writer', 'screenplay', 'author']])
