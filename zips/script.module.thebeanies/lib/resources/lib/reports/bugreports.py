...
# Addon Name: The Beanies
# Addon id: plugin.video.thebeanies
# Addon Provider: The Beanies
# Thanks Muad!
    ...
    class BugReporter_Window(xbmcgui.WindowXMLDialog):
        ...
        def onClick(self, controlId):
            if controlId == self.btn_close:
                ...
                if len(reportText) == 0:
                    xbmc.executebuiltin("XBMC.Notification(%s, %s, %s, %s)" % ('The Beanies Bug Report', 'Bug Report Incomplete', 4000, control.addonIcon()))
                    return
                self.close()
                result = webform.webform().bug_report('The Beanies', reportText)
                if result is None:
                    # Wait before submitting another report you fuckin spammer
                    xbmc.executebuiltin("XBMC.Notification(%s, %s, %s, %s)" % ('The Beanies Bug Report', 'Wait Before Next Submission', 4000, control.addonIcon()))
                elif result is False:
                    # Failed to send. Site timed out or is down. Ya'll suck
                    xbmc.executebuiltin("XBMC.Notification(%s, %s, %s, %s)" % ('The Beanies Bug Report', 'Submission Failed', 4000, control.addonIcon()))
                elif result is True:
                    # Bug Report worked, FUCK YEAH!
                    xbmc.executebuiltin("XBMC.Notification(%s, %s, %s, %s)" % ('The Beanies Bug Report', 'Bug Report Completed', 4000, control.addonIcon()))
