...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.thebeanies.artwork)'):
        return os.path.join(
            xbmcaddon.Addon('script.thebeanies.artwork').getAddonInfo('path'),
            'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility(
        'System.HasAddon(script.thebeanies.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.thebeanies.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.thebeanies.metadata)'):
        return os.path.join(
            xbmcaddon.Addon('script.thebeanies.metadata').getAddonInfo('path'),
