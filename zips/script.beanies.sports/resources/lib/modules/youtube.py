...
class youtube(object):
    ...
    def videos(self, url, pagination=False):
        ...
        key = self.key_link.split('=')[-1]
        import control
        if control.addon('plugin.video.youtube').getSetting('youtube.api.id') == 'thebeanies-trailer':
            control.busy()
            control.addon('plugin.video.youtube').setSetting('youtube.api.key', key)
            control.addon('plugin.video.youtube').setSetting('youtube.api.secret', key)
            control.refresh()
            control.idle()
        elif control.addon('plugin.video.youtube').getSetting('youtube.api.key') == '':
            control.busy()
            control.addon('plugin.video.youtube').setSetting('youtube.api.key', key)
            control.addon('plugin.video.youtube').setSetting('youtube.api.secret', key)
            control.addon('plugin.video.youtube').setSetting('youtube.api.id', 'thebeanies-trailer')
            control.refresh()
            control.idle()
