...
class get(object):
    ...
    def worker(self, url):
        try:
            ...
            r = [i for i in r if hasattr(i[1], 'info') and u in eval(
                i[0])().info()['netloc']][0][0]
            ...
            try:
                ...
                h = dict('')
            h.update(
                {'Referer': url, 'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; rv:34.0) Gecko/20100101 Firefox/34.0'})
...
class captcha:
    ...
    def solvemedia(self, data):
        try:
            ...
            post = {}
            f = common.parseDOM(result, "form", attrs={
                                "action": "verify.noscript"})[0]
            k = common.parseDOM(f, "input", ret="name",
                                attrs={"type": "hidden"})
            for i in k:
                post.update(
                    {i: common.parseDOM(f, "input", ret="value", attrs={"name": i})[0]})
            post.update({'adcopy_response': response})

            getUrl('http://api.solvemedia.com/papi/verify.noscript',
                   post=urllib.urlencode(post)).result

            self.captcha.update(
                {'adcopy_challenge': post['adcopy_challenge'], 'adcopy_response': 'manual_challenge'})
        except:
            pass
    ...
    def recaptcha(self, data):
        try:
            ...
            if data.startswith('http://www.google.com'):
                url += [data]
            url += common.parseDOM(data, "script", ret="src",
            ...
            challenge = re.compile(
                "challenge\s+:\s+'(.+?)'").findall(result)[0]
            response = 'http://www.google.com/recaptcha/api/image?c=' + challenge
            response = self.keyboard(response)

            self.captcha.update({'recaptcha_challenge_field': challenge, 'recaptcha_challenge': challenge,
    ...
    def numeric(self, data):
        try:
            url = re.compile(
                "left:(\d+)px;padding-top:\d+px;'>&#(.+?);<").findall(data)
...
class unwise:
    ...
    def __unwise(self,  w, i, s, e):
        ...
        if 'eval(function(w,i,s,e)' in ret:
            ret = re.compile(
                'eval\(function\(w,i,s,e\).*}\((.*?)\)').findall(ret)[0]
...
class premiumize:
    ...
    def hosts(self):
        try:
            ...
            url = 'http://api.premiumize.me/pm-api/v1.php?method=hosterlist&params[login]=%s&params[pass]=%s' % (
    ...
    def resolve(self, url):
        try:
            ...
            url = 'http://api.premiumize.me/pm-api/v1.php?method=directdownloadlink&params[login]=%s&params[pass]=%s&params[link]=%s' % (
                self.user, self.password, urllib.quote_plus(url))
...
class realdebrid:
    ...
    def resolve(self, url):
        try:
            ...
            url = url.replace('filefactory.com/stream/',
    ...
    def resolve(self, url):
        try:
            if 'picasaweb' in url.lower():
                ...
                oid = re.compile(
                    '/(\d*)/').findall(urlparse.urlparse(url).path)[0]
                key = urlparse.parse_qs(urlparse.urlparse(url).query)[
                    'authkey'][0]

                url = 'http://plus.google.com/photos/%s/albums/%s/%s?authkey=%s' % (
                    oid, aid, pid, key)

            result = getUrl(url, mobile=True).result

            u = re.compile(
                '"(http[s]*://.+?videoplayback[?].+?)"').findall(result)[::-1]
...
class mailru:
    ...
    def resolve(self, url):
        try:
            ...
            url = 'http://videoapi.my.mail.ru/videos/mail/%s/_myvideo/%s.json?ver=0.2.60' % (
                usr, vid)
            ...
            url = []
            try:
                url += [[{'quality': '1080p', 'url': i['url'] + h}
                         for i in u if i['key'] == '1080p'][0]]
            except:
                pass
            try:
                url += [[{'quality': 'HD', 'url': i['url'] + h}
                         for i in u if i['key'] == '720p'][0]]
            except:
                pass
            try:
                url += [[{'quality': 'SD', 'url': i['url'] + h}
                         for i in u if not (i['key'] == '1080p' or i['key'] == '720p')][0]]
            except:
                pass
...
class mybeststream:
    ...
    def resolve(self, url):
        try:
            referer = urlparse.parse_qs(urlparse.urlparse(url).query)[
                'referer'][0]
            page = url.replace(referer, '').replace(
...
class youtube:
    ...
    def resolve(self, url):
        try:
            ...
            message = common.parseDOM(result, "div", attrs={
                                      "id": "unavailable-submessage"})
            message = ''.join(message)

            alert = common.parseDOM(result, "div", attrs={
