...
def get_games():
    ...
    ncaab = soup.select("#nfl")
    if ncaab:
        divs = ncaab[0].select(".col-12.w3-text-white.w3-small")
        for div in divs:
            title = div.select(".w3-center")[0].text.strip()
            links = div.select("a")
            for link in links:
                if ("?v=nfl" not in link['href'].encode("ascii")):
                    continue
                else:
                    url = link['href']
...
def get_stream(link):
    ...
    if iframe:
        ...
        if encrypt:
            ...
            stream.append({"title":"stream","link":decrypt + "|Referer=" + iframe})
        else:
            pass
    else:
        ...
        if iframe:
            ...
            if encrypt:
                ...
                decrypt = decrypt + "|Referer=" + iframe
