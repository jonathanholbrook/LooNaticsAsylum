...
def get_tmdb_data(new_name_array,html_g,fav_search_f,fav_servers_en,fav_servers,google_server,rapid_server,direct_server,heb_server,url,isr,xxx):
           ...
           for data in html['results']:
             ...
             trailer = "plugin://plugin.video.loonatics3000?mode2=25&url=www&id=%s&tv_movie=%s" % (id,tv_movie)
...
def get_all_data(first,last,url,link,new_name_array,isr):
    try:
        ...
        if Addon.getSetting("dp")=='true' and (last-first)>1:
                ...
                dp.create("[COLORspringgreen]Loading... [/COLOR]", "[COLOR deepskyblue]Please[/COLOR] [COLOR blue]Standby[/COLOR]", '')
                dp.update(0)
        thread=[]
...
err :[COLOR aqua]%s[/COLOR]
    ...
    for td in thread:
        ...
        if Addon.getSetting("dp")=='true' and (last-first)>1:
                elapsed_time = time.time() - start_time
                dp.update(0, '[COLORdeepskyblue] Activating [/COLOR]'+ time.strftime("%H:%M:%S", time.gmtime(elapsed_time)),td.name," ")
...
def get_multi_year(url,page,time_dialation):
    ...
    addDir3('[COLORdeepskyblue][I]Next Page[/I][/COLOR]'.decode('utf8'),url_o,133,'https://i.ibb.co/TMJqTVN/next.png','https://i.ibb.co/HnYKbp4/fanart2.jpg','Next Page'.decode('utf8'),data=str(time_dialation),original_title=str(int(match[0])+1))
#    addDir3('[COLORdeepskyblue][I]Next Page[/I][/COLOR]'.decode('utf8'),url_o,133,BASE_LOGO+'next.png',BASE_LOGO+'fanart.jpg','Next Page'.decode('utf8'),data=str(time_dialation),original_title=str(int(match[0])+1))

    #addLink( '[COLORbluei][I]Return to Start[/I][/COLOR]'.decode('utf8'), 'www',103,False,'https://i.ibb.co/CJLrddS/return.png','https://i.ibb.co/HnYKbp4/fanart2.jpg','Return to Start'.decode('utf8'))
...
def get_movies(url,isr,reco=0,new_name_array=[]):
   ...
   if reco==0:
     addDir3('[COLORdeepskyblue][I]Next Page[/I][/COLOR]'.decode('utf8'),link+'page='+str(int(match[0])+1),3,'https://i.ibb.co/TMJqTVN/next.png','https://i.ibb.co/HnYKbp4/fanart2.jpg','Next Page'.decode('utf8'),isr=isr)
#     addDir3('[COLORdeepskyblue][I]Next Page[/I][/COLOR]'.decode('utf8'),link+'page='+str(int(match[0])+1),3,BASE_LOGO+'next.png',BASE_LOGO+'fanart.jpg','Next Page'.decode('utf8'),isr=isr)
