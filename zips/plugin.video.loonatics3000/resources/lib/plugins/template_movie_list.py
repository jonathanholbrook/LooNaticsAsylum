...
class Template_Movie_List(Plugin):
    ...
    def process_item(self, item_xml):
        if "<temp_db_movie>" in item_xml:
            item = JenItem(item_xml)
            if "movies" in item.get("temp_db_movie", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
                    'mode': "open_template_movies",
                    'url': "",
                    'folder': True,
                    'imdb': "0",
                    'season': "0",
            ...
            elif "genre" in item.get("temp_db_movie", ""):    
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
                    'mode': "open_template_genre_movies",
                    'url': item.get("temp_db_movie", ""),
                    'folder': True,
                    'imdb': "0",
                    'season': "0",
                    'episode': "0",
...
@route(mode='open_template_movies')
def open_movies():
    xml = ""
