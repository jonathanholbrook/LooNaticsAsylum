...
def imdbmovies(url):
    ...
    try:
        ...
        xml += "<dir>"\
               "<title>[COLORdeepskyblue]Next Page[/COLOR]</title>"\
               "<imdburl>http://www.imdb.com/search/title%s</imdburl>"\
               "<thumbnail>https://i.ibb.co/TMJqTVN/next.png</thumbnail>"\
               "</dir>" % (next_page)
    except:
        pass
    jenlist = JenList(xml)
...
def imdbseason(url):
    ...
    for imdb, season in match2:
        ...
        name = "Season: [COLOR lightseagreen]" + season + "[/COLOR]"
        xml +=  "<dir>"\
                "<title>%s</title>"\
                "<meta>"\
...
def imdbepisode(url):
    ...
    for imdb, title, thumbnail, season, episode, premiered in match:
            ...
            name = "[COLOR lightseagreen]%sx%s[/COLOR] . %s" % (season, episode, title)
            xml +=  "<item>"\
                    "<title>%s</title>"\
                    "<meta>"\
