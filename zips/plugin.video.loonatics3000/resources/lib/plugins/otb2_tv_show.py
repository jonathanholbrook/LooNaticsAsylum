...
----------------------------------------------------------
"""
table_id = "appYNvUY6GSnwkbuW"
table_name = "tv shows"
workspace_api_key = "keyYbkT9MVResyqDf"
"""
----------------------------------------------------------
"""
...
class OTB2_TV(Plugin):
    name = "otb2_tv"

    def process_item(self, item_xml):
        if "<otb2_tv>" in item_xml:
            item = JenItem(item_xml)
            if "all" in item.get("otb2_tv", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
                    'mode': "open_otb2_tv_shows",
                    'url': item.get("otb2_tv", ""),
                    'folder': True,
                    'imdb': "0",
                    'season': "0",
                    'episode': "0",
                ...
                return result_item              
            elif "show|" in item.get("otb2_tv", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
                    'mode': "db_open_otb2_selected_show",
                    'url': item.get("otb2_tv", ""),
                    'folder': True,
                    'imdb': "0",
                    'season': "0",
                ...
                result_item['fanart_small'] = result_item["fanart"]
                return result_item 
            elif "season|" in item.get("otb2_tv", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
                    'mode': "open_otb2_season",
                    'url': item.get("otb2_tv", ""),
                    'folder': True,
                    'imdb': "0",
                    'season': "0",
...
@route(mode='open_otb2_tv_shows')
def open_otb2_tv_shows():
    xml = ""
...
@route(mode='db_open_otb2_selected_show',args=["url"])
...
@route(mode='open_otb2_season',args=["url"])
def open_selected_show(url):
    xml = ""
    title = url.split("|")[-3]
    key = url.split("|")[-2]
