...
class System(object):
	...
	GaiaAddon = 'plugin.video.loonatics3000'
	...
	def restart(self, sleep = True):
		...
		System.execute('RunAddon(%s)' % 'plugin.video.loonatics3000')
	...
	@classmethod
	def info(self, value):
		return xbmcaddon.Addon('plugin.video.loonatics3000').getAddonInfo(value)
	...
	def window(self, action = None, parameters = {}, command = None, sleep = None, refresh = False):
		if command == None:
			...
			if not parameters == None and not parameters == '' and not parameters == {}:
				...
				parameters = ''
			command = '%s%s/%s' % (System.PluginPrefix, 'plugin.video.loonatics3000', parameters)
...
SettingsAddon = xbmcaddon.Addon('plugin.video.loonatics3000')
...
class Media(object):
	...
	NameSeasonLong = 'season'#xbmcaddon.Addon('plugin.video.loonatics3000').getLocalizedString(32055).encode('utf-8')
	NameSeasonShort = NameSeasonLong[0].upper()
	NameEpisodeLong = 'S'#xbmcaddon.Addon('plugin.video.loonatics3000').getLocalizedString(33028).encode('utf-8')
