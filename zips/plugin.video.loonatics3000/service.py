...
while not xbmc.abortRequested:
    ...
    if xbmc.Player().isPlaying():
        ...
        if '_from_Loonatics 3000_' in current_list_item and '-KIDSSECTION-' not in current_list_item and '__aceplay__' not in current_list_item:
           try:
       ...
       if counter_no_play==5:
        if 'channel' not in current_list_item and '_from_Loonatics 3000_' in current_list_item:
            result = xbmc.executeJSONRPC('{"jsonrpc": "2.0", "method": "Playlist.GetItems", "params": { "properties": [ "showlink", "showtitle", "season", "title", "artist" ], "playlistid": 1}, "id": 1}')
            j_list=json.loads(result)
            playlist = xbmc.PlayList(xbmc.PLAYLIST_VIDEO)
            playlist.clear()
