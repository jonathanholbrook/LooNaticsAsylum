...
addon_name = 'The Beanies'
addon_icon = xbmcaddon.Addon().getAddonInfo('icon')
addon_path = xbmc.translatePath(
    ('special://home/addons/plugin.video.thebeanies')).decode('utf-8')
module_path = xbmc.translatePath(
    ('special://home/addons/script.module.thebeanies')).decode('utf-8')
...
def main():
    fum_ver = xbmcaddon.Addon(
        id='script.module.thebeanies').getAddonInfo('version')
    updated = xbmcaddon.Addon(
        id='plugin.video.thebeanies').getSetting('module_base')
...
####FREE####
    ...
    log('The Beanies Scraper Path: %s' % (str(scraper_path)), LOGINFO)
    try:
        xml = openfile(settings_xml_path)
    except Exception:
        failure = traceback.format_exc()
        log('The Beanies Service - Exception: \n %s' % (str(failure)), LOGINFO)
        return
    ...
    xbmcaddon.Addon(id='plugin.video.thebeanies').setSetting(
        'module_base', fum_ver)
...
DEBUGPREFIX = '[COLOR red][ The Beanies DEBUG ][/COLOR]'
...
def syncTraktLibrary():
    control.execute(
        'RunPlugin(plugin://%s)' % 'plugin.video.thebeanies/?action=tvshowsToLibrarySilent&url=traktcollection')
    control.execute(
        'RunPlugin(plugin://%s)' % 'plugin.video.thebeanies/?action=moviesToLibrarySilent&url=traktcollection')
